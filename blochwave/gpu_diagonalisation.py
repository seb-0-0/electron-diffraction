...
class Bloch:
    ...
    def set_beam(self,
        ...
        if isinstance(K,list) or isinstance(K,np.ndarray):
            ...
            if isinstance(u0,list) or isinstance(u0,np.ndarray):
                ...
                u = np.array(u0.get()).dot(self.lat_vec)
            #print(type(u))
            u = np.array(u)
    ...
    def _solve_Bloch(self,show_H=False,Vopt0=True,dyngo_args={},v=False):
        ...
        self.gammaj,self.CjG = cp.linalg.eigh(cp.array(self.H)) #;print(red+'Ek',lk,black);print(wk)
    ...
    def _set_beams_vs_thickness(self,thicks=None):
        ...
        print(type(np.outer(cp.array(gammaj),cp.array(self.z))))
        M = np.exp(1J*np.outer(cp.array(gammaj),cp.array(self.z)))*(invCjG[:,id0][:,None])
        St = CjG.dot(M)
